        ...
        {tab === 'planner' && (
          <div className="space-y-4">
            <input
              ...
              className="border rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-indigo-400 outline-none"
            />
            <button
              onClick={generateCourse}
              className="px-4 py-2 rounded-full bg-gradient-to-r from-green-400 to-teal-500 text-white font-semibold shadow hover:opacity-90 transition"
            >
              Generate Outline
            </button>
        ...
        {tab === 'studio' && (
          <div className="space-y-4">
            <div className="relative rounded-xl overflow-hidden shadow-lg">
              ...
              {recording && (
                <div className="absolute top-3 left-3 bg-red-600 text-white px-4 py-1 rounded-full font-semibold shadow">
                  {paused ? 'PAUSED' : 'REC 🔴'}
                </div>
              )}
            </div>
            ...
            {!recording ? (
              <button
                onClick={startStudio}
                className="px-5 py-2 rounded-full bg-gradient-to-r from-green-400 to-teal-500 text-white font-semibold shadow hover:opacity-90 transition"
              >
                Start Recording
              </button>
            ) : (
              <div className="flex gap-3">
                <button
                  onClick={pauseResume}
                  className="px-5 py-2 rounded-full bg-yellow-400 text-white font-semibold shadow hover:opacity-90 transition"
                >
                  {paused ? 'Resume' : 'Pause'}
                </button>
                <button
                  onClick={stopStudio}
                  className="px-5 py-2 rounded-full bg-red-500 text-white font-semibold shadow hover:opacity-90 transition"
                >
                  Stop
                </button>
              </div>
        ...
        {tab === 'transcript' && (
          <div className="space-y-3">
            <button
              ...
              className="px-4 py-2 rounded-full bg-gradient-to-r from-purple-500 to-indigo-500 text-white font-semibold shadow hover:opacity-90 transition"
            >
              Start Transcription
            </button>
            <textarea
              value={transcript}
              readOnly
              className="w-full h-64 border p-3 rounded-lg bg-gray-50 focus:ring-2 focus:ring-indigo-400 outline-none"
            ></textarea>
          </div>
        )}
        ...
        {tab === 'thumbnail' && (
          <div className="space-y-3">
            <input
              ...
              className="border px-3 py-2 w-full rounded-lg focus:ring-2 focus:ring-indigo-400 outline-none"
            />
            <div className="h-32 flex items-center justify-center border-2 border-dashed rounded-lg text-gray-400 font-semibold bg-gradient-to-br from-gray-50 to-gray-100">
              {thumbText || 'Thumbnail Preview'}
            </div>
          </div>
        )}
        ...
        {tab === 'library' && (
          <div className="space-y-3">
            {clips.length === 0 ? (
              ...
              clips.map((c, i) => (
                <video key={i} src={c} controls className="w-full rounded-xl shadow" />
              ))
            )}
          </div>
        )}
