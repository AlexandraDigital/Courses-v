...
export default function App() {
  ...
  const [course, setCourse] = useState({
    topic: localStorage.getItem('courseTopic') || '',
    weeks: 4,
    videos: 3
  });
  ...
  useEffect(() => {
    ...
    localStorage.setItem('courseTopic', course.topic);
    localStorage.setItem('thumbText', thumbText);
  }, [transcript, course.topic, thumbText]);
  ...
  const startStudio = async () => {
    try {
      ...
      try { screenStream = await navigator.mediaDevices.getDisplayMedia({ video: true }); } catch {}
        ...
        {tab==='thumbnail' && (
          <div>
            ...
            <div className='thumb-preview'>
              {thumbText || 'AI Thumbnail Preview'}
            </div>
          </div>
        )}
